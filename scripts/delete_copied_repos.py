...
def delete_copied_repos(root_dir: str, dry_run: bool = False):
    root_path = Path(root_dir).resolve()
    total_size = 0
    deleted_files = []

    for path in root_path.rglob("weirdl-pomdp-baselines"):
        if path.is_dir() and path.parent.parent.name.startswith("nov"):
            try:
...
if __name__ == "__main__":
    ...
    delete_copied_repos(args.root_dir, dry_run=args.dry_run)
